          ...
          <div className="space-y-2">
            <div className="relative">
              <button
                ...
                className="w-full text-left px-3 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 hover:bg-slate-50 dark:hover:bg-slate-600 transition-all duration-200 text-sm font-medium text-slate-900 dark:text-white truncate hover:shadow-md focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
              >
                {selectedStory ? selectedStory.title : 'Select a story...'}
              </button>

              {/* Dropdown (Story 5.5: Enhanced with animations) */}
              {showStorySelector && (
                <div className="absolute z-20 mt-1 w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-xl max-h-60 overflow-y-auto animate-in fade-in slide-in-from-top-2 duration-200">
                  {stories.map((story) => (
                    <button
                      key={story._id}
                      onClick={() => {
                      ...
                      className={`w-full text-left px-3 py-2 text-sm hover:bg-slate-100 dark:hover:bg-slate-700 transition-all duration-150 ${
