...
<div className="scene-item flex justify-between items-center p-2">
  ...
  <div className="flex gap-1">
    ...
    {scene.regenerationCount > 0 && <span className="text-xs bg-yellow-100">×{scene.regenerationCount}</span>}
    {scene.prose && <span className="text-xs text-slate-500">{wordCount(scene.prose)}</span>}
  </div>
</div>
```
...
```tsx
<GenerationStatus currentStep={1 | 2} characters={loadedCharacters} elapsedTime={timeInMs} isError={false} />
```
